...
class LocationService:
	...
	def get_object_location(
		...
		"""
		logger.info(f"Начинаю поиск объекта на карте {object_type.name} {folder_name}...")

		match object_type:
			case cnst.ObjectTypeEnum.buttons:
				files = [os.getcwd() + f'/app/img/{object_type.value}/{folder_name}.png']
			case cnst.ObjectTypeEnum.enemies:
				img_folder = os.getcwd() + f'/app/img/{object_type.value}/{folder_name}'
				files = [join(img_folder, x) for x in os.listdir(img_folder) if isfile(join(img_folder, x))]
			case _:
	...
	def _get_location_or_none(files: list[str], confidence: float = 0.8):
		for file in files:
			try:
				location = pyautogui.locateOnScreen(file, confidence=confidence, region=region, grayscale=True)
				return location
	...
	def _get_location(folder_name: str, files: list[str], confidence: float = 0.8):
		...
		logger.info(f'======== Не Удалось найти объект {folder_name} c первого раза, пробую еще =======')
		# Сместить фокус
		pyautogui.moveRel(
	...
	def click(location, duration: float | None = None, is_double: bool = False) -> None:
		...
		pyautogui.moveTo(
			...
			duration=duration if duration is not None else random.randrange(100, 300) / 1000
		)
